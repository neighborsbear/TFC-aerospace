...
public class RingsEffectProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, Entity entity) {
		if (entity == null)
			return;
		if ((entity.getCapability(TfcAerospaceModVariables.PLAYER_VARIABLES_CAPABILITY, null)
				.orElse(new TfcAerospaceModVariables.PlayerVariables())).aerospacehat) {
			if (entity instanceof LivingEntity _entity)
				_entity.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, 60, 1));
		}
		if ((entity.getCapability(TfcAerospaceModVariables.PLAYER_VARIABLES_CAPABILITY, null)
				.orElse(new TfcAerospaceModVariables.PlayerVariables())).aerospaceshirt) {
			if (entity instanceof LivingEntity _entity)
